...
SELECT smsr.name,
   ...
   FROM steam_item_sale.skinbaron_market_search_results smsr
     JOIN steam_item_sale.steam_most_recent_prices smrp ON smsr.name::text = smrp.name::text AND smrp.price_euro <> 0::double precision AND smsr.name::text !~~ '1Sticker |%'::text AND smsr.name::text !~~ 'Souvenir%'::text AND smsr.name::text !~~ 'Sealed Graffiti%'::text AND smrp.quantity > 100
