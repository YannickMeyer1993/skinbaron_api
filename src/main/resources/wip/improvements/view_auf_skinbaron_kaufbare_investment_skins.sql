...
AS SELECT smsr.id,
   ...
   FROM steam_item_sale.skinbaron_market_search_results smsr
     JOIN steam.steam_current_prices smrp ON smsr.name::text = smrp.name::text AND smrp.price_euro <> 0::double precision AND smsr.name::text !~~ 'Souvenir%'::text AND smsr.name::text !~~ 'Sealed Graffiti%'::text AND smrp.quantity > 25
